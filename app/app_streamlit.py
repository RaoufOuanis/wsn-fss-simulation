...
from wsn.multihop import MultiHopParams
...
st.caption(
    "Cadre: clustering centralisé, single-hop ou multi-hop CH→sink, énergie radio (LEACH-style), métriques FND/HND/LND + throughput."
)
...
with st.sidebar.expander("Multi-hop CH→sink", expanded=False):
    multihop_enabled = st.checkbox("Enable multi-hop (CH→CH→sink)", value=True)
    r_tx = st.slider("Transmission radius r_tx", 5.0, 200.0, 50.0, 1.0)
    w_relay = st.slider("Relay-load weight w_relay", 0.0, 0.5, 0.05, 0.01)
...
fit_params = FitnessParams(
    ...
    lam=float(lam),
    rc=float(rc),
    multihop=bool(multihop_enabled),
    r_tx=float(r_tx),
    w_relay=float(w_relay),
    radio=rp,
)
...
def _init_interactive_state(force: bool = False) -> None:
    ...
    st.session_state.pkts_history = []  # legacy: pkts_to_sink per round
    st.session_state.delivered_history = []
    st.session_state.pkt_hops_history = []
    st.session_state.throughput = 0  # delivered reports cumulative
...
def _interactive_one_round(algo: str) -> None:
    ...
    mh = MultiHopParams(r_tx=float(getattr(fit_params, "r_tx", fit_params.rc))) if bool(getattr(fit_params, "multihop", False)) else None
    stats = apply_round_energy(net, ch, assignments, rp, multihop=mh)
    ...
    pkt_hops_round = int(stats.get("pkt_hops", pkts_to_sink_round))

    st.session_state.pkts_history.append(pkts_to_sink_round)
    st.session_state.delivered_history.append(delivered_round)
    st.session_state.pkt_hops_history.append(pkt_hops_round)
    st.session_state.throughput += delivered_round
...
def _simulate_one(
    ...
    pkts_hist: List[int] = []
    delivered_hist: List[int] = []
    pkt_hops_hist: List[int] = []
    throughput_cum = 0
    ...
    for r in range(int(max_rounds)):
        ...
        if algo.startswith("FSS"):
            ...
            nfe_total += float(getattr(res, "nfe", 0.0))
            ...
            nfe_total += float(getattr(res, "nfe", 0.0)) if float(getattr(res, "nfe", 0.0)) > 0 else float(params.n_iter * params.n_particles)
            ...
            nfe_total += float(getattr(res, "nfe", 0.0)) if float(getattr(res, "nfe", 0.0)) > 0 else float(params.n_iter * params.n_wolves)
            ...
            nfe_total += float(getattr(res, "nfe", 0.0)) if float(getattr(res, "nfe", 0.0)) > 0 else float((2 * params.n_iter + 1) * params.n_food_sources)
        ...
        mh = MultiHopParams(r_tx=float(getattr(fit_params_local, "r_tx", fit_params_local.rc))) if bool(getattr(fit_params_local, "multihop", False)) else None
        stats = apply_round_energy(net, ch, assignments, radio_local, multihop=mh)
        ...
        pkts_hist.append(pkts_to_sink_round)
        delivered_hist.append(delivered_round)
        pkt_hops_hist.append(pkt_hops_round)
        throughput_cum += delivered_round
    ...
    ts = pd.DataFrame(
        {
            ...
            "delivered_reports": np.asarray(delivered_hist, dtype=int),
            "pkts_to_sink": np.asarray(pkts_hist, dtype=int),
            "pkt_hops": np.asarray(pkt_hops_hist, dtype=int),
        }
    )
    ts["throughput_cum"] = ts["delivered_reports"].cumsum()
    ts["pkts_to_sink_cum"] = ts["pkts_to_sink"].cumsum()
    ts["pkt_hops_cum"] = ts["pkt_hops"].cumsum()
    return summary, ts
...
def _run_compare_cached(config_key: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    fit_params_local = FitnessParams(
        ...
        multihop=bool(cfg["fitness"].get("multihop", True)),
        r_tx=float(cfg["fitness"].get("r_tx", 50.0)),
        w_relay=float(cfg["fitness"].get("w_relay", 0.05)),
    )
    radio_local = RadioParams(**cfg["radio"])
    fit_params_local.radio = radio_local
    ...
    if mode != "Interactive (single algo)":
        ...
        with right:
            ...
            if len(rounds) == 0:
                ...
                pkts_to_sink_total = int(np.sum(np.asarray(st.session_state.pkts_history, dtype=int))) if st.session_state.pkts_history else 0
                pkt_hops_total = int(np.sum(np.asarray(st.session_state.pkt_hops_history, dtype=int))) if st.session_state.pkt_hops_history else 0

                st.markdown(f"**Throughput total (delivered reports):** {throughput_total}")
                st.markdown(f"**Pkts to sink (legacy, CH→sink):** {pkts_to_sink_total}")
                st.markdown(f"**Packet-hops (multi-hop load):** {pkt_hops_total}")
    ...
    if mode != "Compare (multi algo, full runs)":
        ...
        if isinstance(df_sum, pd.DataFrame) and not df_sum.empty:
            ...
            if isinstance(df_ts, pd.DataFrame) and not df_ts.empty:
                ...
                metric = st.selectbox(
                    "Metric",
                    ["alive", "total_energy", "throughput_cum", "pkts_to_sink_cum", "pkt_hops_cum"],
                    index=0,
            ...
            df_session = pd.DataFrame(
                {
                    ...
                    "delivered_reports": st.session_state.delivered_history,
                    "pkts_to_sink": st.session_state.pkts_history,
                    "pkt_hops": st.session_state.pkt_hops_history,
                }
            )
            df_session["throughput_cum"] = df_session["delivered_reports"].cumsum()
            df_session["pkts_to_sink_cum"] = df_session["pkts_to_sink"].cumsum()
            df_session["pkt_hops_cum"] = df_session["pkt_hops"].cumsum()
