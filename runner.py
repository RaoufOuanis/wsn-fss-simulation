...
S1_ALGOS_DEFAULT = "FSS,PSO,GWO,ABC,LEACH,HEED,Greedy"
S2_ALGOS_DEFAULT = "FSS,PSO,GWO,ABC,SEP,Greedy"
...
class RunnerApp:
    ...
    def _rebuild_metric_widgets(self) -> None:
        ...
        # default: select the current metric in checkboxes too
        cur = self.metric_var.get().strip()
        if cur not in vals:
            cur = vals[0]
            self.metric_var.set(cur)
        if cur in self._metric_select_vars:
            self._metric_select_vars[cur].set(True)
