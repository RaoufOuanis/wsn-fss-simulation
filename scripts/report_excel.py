...
MULTIHOP_METRICS_ORDER = [
    # kit minimal multi-hop (diagnostic)
    "pkt_hops",
    "mh_avg_path_hops",
    "mh_q_max",
    "mh_jain_q",
...
def _build_ordered_stats_table(summary_df: pd.DataFrame, metrics: List[str]) -> pd.DataFrame:
    """Return a SummaryStats-like table but restricted to metrics in the given order."""
    ...
    mean_df = g[present].mean(numeric_only=True).add_suffix("_mean")
    std_df = g[present].std(numeric_only=True).add_suffix("_std")
    n_df = g.size().rename("n")

    out = pd.concat([n_df, mean_df, std_df], axis=1).reset_index()

    # Reorder columns deterministically
    cols: List[str] = ["algo", "n"]
    for m in present:
        cols.append(m + "_mean")
        cols.append(m + "_std")
...
def _metric_catalog(summary_df: pd.DataFrame) -> pd.DataFrame:
    """Small helper sheet to make reports reviewer-friendly."""
    ...
    for m in metrics:
        rows.append(
            {
                "metric": m,
                "label": metric_display_name(m),
                "direction": direction_phrase(m),
                "group": (
                    "core" if m in CORE_METRICS_ORDER else "multihop" if m in MULTIHOP_METRICS_ORDER else "other"
                ),
            }
        )
...
def main() -> None:
    ...
    main_table_df = _build_ordered_stats_table(summary_df, CORE_METRICS_ORDER)
    mh_table_df = _build_ordered_stats_table(summary_df, MULTIHOP_METRICS_ORDER)
    catalog_df = _metric_catalog(summary_df)
    ...
    try:
        with pd.ExcelWriter(paths.out, engine="openpyxl") as writer:
            ...
            # Reviewer-friendly, stable-order tables
            if not main_table_df.empty:
                main_table_df.to_excel(writer, sheet_name="MainTable", index=False)
            if not mh_table_df.empty:
                mh_table_df.to_excel(writer, sheet_name="MultiHopTable", index=False)
            if not catalog_df.empty:
                catalog_df.to_excel(writer, sheet_name="MetricCatalog", index=False)
